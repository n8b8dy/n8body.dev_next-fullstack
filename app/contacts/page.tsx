...
const Paragraphs = [
  ...
  { text: 'contact@n8body.dev', icon: <TbMailCode/> },
  { text: 'ruslanshamsutdinov.dev@gmail.com', icon: <TbMail/> },
] as const
      ...
      <div className={cn('px-2 py-4 lg:flex-[0_0_48%]')}>
        <div className={cn('max-w-lg mx-auto')}>
          ...
          <div className={cn('pb-4 flex flex-col gap-4 overflow-x-auto')}>
            {Paragraphs.map(({ text, icon }) => <div key={text} className={cn('flex items-center gap-1')}>
              <span className={cn('text-3xl mr-1')}>{icon}</span>
              <span className={cn('text-xl')}>{text}</span>
            </div>)}
          </div>
          ...
          <div className={cn('flex flex-wrap gap-2')}>
            {Links.map(({ text, href, icon }) => <Link key={href} href={href} className={cn(
              'px-2 py-3 flex justify-center items-center gap-3 rounded transition',
              'bg-action-button',
            )}>
              <span className={cn('text-2xl')}>{icon}</span>
              <span className={cn('text-xl text-wrap')}>{text}</span>
            </Link>)}
          </div>
        </div>
      </div>
